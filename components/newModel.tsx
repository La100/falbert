...
const NewModel: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // Zmieniony sposób tworzenia pliku ZIP
      console.log('Przygotowywanie pliku ZIP...');
      const zip = new JSZip();
      ...
      // Generuj ZIP jako Blob z odpowiednim typem MIME
      ...
      // Utwórz plik z poprawnym typem MIME
      const zipFile = new File([zipBlob], 'images.zip', { type: 'application/zip' });

      console.log('Wysyłanie żądania z tokenem:', session.access_token);
      ...
      console.log('Model został pomyślnie przesłany:', result);
  ...
  if (!session) {
    return (
      <div className="bg-gray-900 text-gray-100 p-6 rounded-lg max-w-2xl mx-auto">
        <p className="text-center">Musisz być zalogowany, aby trenować modele.</p>
      </div>
    );
  }
